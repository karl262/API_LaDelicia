...
export default class Sale {
  ...
  static async findAll() {
    const result = await pool.query(
      "SELECT * FROM sales WHERE delete_at IS NULL"
    ); // Filtrar eliminados
    return result.rows;
  }
  ...
  static async delete(id) {
    const result = await pool.query(
      "UPDATE sales SET delete_at = CURRENT_TIMESTAMP WHERE id = $1 RETURNING *",
      [id]
    ); // Soft delete
    return result.rows[0];
  }

  static async getSalesByClientId(clientId) {
    const result = await pool.query(
      "SELECT s.*, sd.* FROM sales AS s JOIN sale_details AS sd ON sd.id = s.id WHERE s.clientid = $1 AND s.delete_at IS NULL",
      [clientId]
    );
    return result.rows;
  }
